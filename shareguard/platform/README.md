...
- `public_demo/`：独立的安全静态案例，只用于讲解产品流程。
- `shareguard/platform/static/`：正式工作台，空白启动，只接受真实 `/v1/analyze` 响应。
- 正式工作台拒绝 `mock` 或 `X-ShareGuard-Demo` 响应；服务不可用时显示“无模型结果”，绝不回填静态结论、定位框或传播拓扑。
- 私有模型服务：按需 GPU 推理，加载只读模型归档；权重和内部参数不进入 GitHub 或浏览器。
